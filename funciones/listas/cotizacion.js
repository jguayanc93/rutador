...
let bd_consulta = (res,vendedor)=>{
    let sp_sql="select CONVERT(varchar,fecha,111),ndocu,nomcli,totn,(case cdge when '' then 'cotizado' when '01' then 'facturado' when '03' then 'boleta' when '32' then 'pedido' end)as 'resultado' from mst01cot where YEAR(fecha)=2025 and MONTH(fecha)=CONVERT(varchar(2),GETDATE(),101) and DAY(fecha)=CONVERT(varchar(2),GETDATE(),103) and codven_usu=@vendedor";
...
let blistacotisxdia = (req,res,next) => {
    // let valid_coki = req.signedCookies;
    let {dia} = req.body;
...
module.exports={chekeador,blistacotis,blistacotisxdia}